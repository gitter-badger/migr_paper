...
Besides the popularity of metaheuristics among the optimization communities, there is also growing interest in exploring parallel models of computation.
...
We propose a methodology for characterizing the migration interplay between three popular metaheuristics, and evaluate the performance on real--parameter single objective optimization benchmarks.
...
Search metaheuristics have been steadily growing in popularity over the recent years, mostly due to their performance in solving global optimization problems.
Although several search techniques are shown to perform well across many problems, no single algorithm can claim to be universal~\cite{wolpert1997no}.
...
\subsection{Probabilistic Migration Model (PMM)}
    ...
    Migration rate $r$ and migration probability $p$ are two ways of parametrizing a single concept, i.e. the rate of information passing from one population to another.%TODO: (CITE TO Cantu PAZ about migration).
    ...
    %Thus, PMM allows for more customization of the migration operator.
    ...
    \subfigure[$2$--heuristic, $4$--island setup ($4$ parameters).]{
        ...
        \node[vertex] (c) at (1, 0) {$H_2$};

        \node[vertex] (d) at (0, 1) {$H_1$};

        \path[edge2] (a) to[near end] node[weight]{$p_3,p_4$} (b) ;
        \path[edge2] (c) to[near end] node[weight]{$p_1,p_2$} (d) ;
        ...
        \path[edge] (b) to[bend right=18] node[weight]{$p_3$} (c);
        \path[edge] (c) to[bend right=18] node[weight]{$p_4$} (b) ;

        \path[edge] (a) to[bend right=18] node[weight]{$p_2$} (d) ;
        \path[edge] (d) to[bend right=18] node[weight]{$p_1$} (a);

        \path[edge] (b) to[bend right=18] node[weight]{$p_2$} (d);
        \path[edge] (d) to[bend right=18] node[weight]{$p_1$} (b) ;
        \end{tikzpicture}
    }\quad
    \subfigure[$3$--heuristic, $3$--island setup ($6$ parameters).]{
        ...
        \path[edge] (b) to[bend right=18] node[weight]{$p_5$} (c);
        \path[edge] (c) to[bend right=18] node[weight]{$p_6$} (b) ;


        \end{tikzpicture}
    }\quad
    \subfigure[Set of pairwise reductions of (c) above.]{

        \begin{tikzpicture}[scale=2.5]
        \tikzstyle{vertex}=[draw,text=black,auto=left,circle,fill=white,minimum size=15pt]
        \tikzstyle{invisible}=[draw,text=white,color=white,fill=white]
        \tikzstyle{vertexred}=[draw,text=black,auto=left,color=red,circle,fill=white,minimum size=15pt]
        \tikzstyle{weight} = [fill=white]
        \tikzstyle{edge} = [draw,thick,->]
        \tikzstyle{edge2} = [draw,thick,-]

        \node[invisible] (i1) at (0.0, 0.0) {hi};
        \node[invisible] (i1) at (1.0, 1.0) {hi};

        \node[vertex] (a) at (0.3, 1) {$H_1$};
        \node[vertex] (b) at (0.2, 0) {$H_2$};

        \node[vertex] (c) at (0.8, 1) {$H_2$};
        \node[vertex] (d) at (0.7, 0) {$H_2$};

        \path[edge] (c) to[bend right=18] node[weight]{$p_3$} (d) ;
        \path[edge] (d) to[bend right=18] node[weight]{$p_4$} (c);
    ...
    \subfigure[Set of pairwise reductions of (b) above.]{
        ...
        \node[vertex] (a) at (0.1, 1) {$H_1$};
        \node[vertex] (b) at (0, 0) {$H_2$};

        \node[vertex] (c) at (0.6, 1) {$H_1$};
        \node[vertex] (d) at (0.5, 0) {$H_3$};

        \node[vertex] (e) at (1.1, 1) {$H_2$};
        \node[vertex] (f) at (1, 0) {$H_3$};

        \path[edge] (e) to[bend right=18] node[weight]{$p_5$} (f) ;
        \path[edge] (f) to[bend right=18] node[weight]{$p_6$} (e);
...
\subsection{Methodology}
    ...
    \label{alg:main_loop}
    \begin{algorithmic}[1]
        \Procedure{Optimize}{$f, P_1, P_2, H_1, H_2, p_1, p_2$}%\Comment{Single solution from population $P_1$ to $P_2$}
        \State $P_1 \gets \Call{Initialize}{f}$
        \State $P_2 \gets \Call{Initialize}{f}$
        \Do
            \State $P_1 \gets \Call{Evolve}{P_1, H_1, f)}$\Comment{Evolve $P_1$, using algorithm $H_2$, on a function $f$}
            \State $P_2 \gets \Call{Evolve}{P_2, H_2, f)}$%\Comment{Evolve population $P$, on a function $f$, using algorithm $h(P)$}

            \State $q_1 \sim U(0,1)$\Comment{Sample uniformly from $(0, 1)$}
            \State $q_2 \sim U(0,1)$%\Comment{Sample uniformly from $(0, 1)$}
            \If{$q_1 < p_1$}%\Comment{Migrate if passed the probability threshold $p_1$}\\
                \State $P_2^* \gets \Call{Migrate}{P_1, P_2}$\Comment{Migrate $P_1 \rightarrow P_2$}
            \EndIf
            \If{$q_2 < p_2$}%\Comment{Migrate if passed the probability threshold $p_1$}\\
                \State $P_1^* \gets \Call{Migrate}{P_2, P_1}$
            \EndIf
            \State $P_1 \gets P_1^*$
            \State $P_2 \gets P_2^*$

            \DoWhile{not $\Call{MeetsStopCriteria}{P_2}$}
            %\State $f_0, FE \gets \Call{GetResults}{P_2}$\Comment{The best solution $f'$ and number of function evaluations $B'$}
...
\begin{figure}[ht!]
    ...
    \node[text width=4.3cm] at (2.1, 0.0) {
...
%For that reason the migration in one--directional setup has much a lesser effect on the running time SEE(pso jde cec 7).
